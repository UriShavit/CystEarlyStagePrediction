...
from CESP.processing.batch_creation import create_batches
from CESP.utils.patch_operations import slice_matrix, concat_matrices, pad_patch, crop_patch

# Class to handle all preprocessing functionalities
class Preprocessor:

    def __init__(self, data_io, batch_size, subfunctions=[], data_aug=Data_Augmentation(), prepare_subfunctions=False,
                 prepare_batches=False, analysis="patchwise-crop", patch_shape=None):
        """
    ...
    and Data IO instance (optional also Data Augmentation instance).
        :param data_io: Data IO class instance which handles all I/O operations according to the user
                                                defined interface.
        :param batch_size: Number of samples inside a single batch.
        :param subfunctions:  List of Subfunctions class instances which will be SEQUENTIALLY executed on the data set.
                                                (clipping, normalization, resampling, ...)
        :param data_aug: Data Augmentation class instance which performs diverse data augmentation techniques.
        ...
        :param prepare_subfunctions: Should all subfunctions be prepared and backup to disk before starting the batch generation
                                    (True), or should the subfunctions preprocessing be performed during runtime? (False).
        :param prepare_batches: Should all batches be prepared and backup to disk before starting the training (True),
                                or should the batches be created during runtime? (False).
        :param analysis: Modus selection of analysis type. Options:
        ...
        :param patch_shape: Size and shape of a patch. The variable has to be defined as a tuple.
