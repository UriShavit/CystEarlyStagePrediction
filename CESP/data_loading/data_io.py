...
import shutil
import pickle
import CESP.data_loading.sample as CESP_sample
...
class Data_IO:
    ...
    def __init__(self, interface, input_path, output_path="predictions",
        ...
        :param interface: A data IO interface which inherits the abstract_io class with the following methods:
                                    initialize, load_image, load_segmentation, load_prediction, save_prediction
        :param input_path: Path to the input data directory, in which all imaging data have to be accessible.
        :param output_path: Path to the output data directory, in which computed predictions will be stored.
                            This directory will be created.
        :param batch_path: Path to the batch data directory. This directory will be created and used for temporary files.
        delete_batchDir (boolean):  Boolean if the whole temporary batch directory for prepared batches should be
                                    deleted after
        :param delete_batchDir:  Boolean if the whole temporary batch directory for prepared batches should be deleted
                                 after  model utilization. If false only the batches with the associated seed will be
                                 deleted. This parameter is important when running multiple instances of MIScnn.
        """
    ...
    def sample_loader(self, index, load_seg=True, load_pred=False, backup=False):
        ...
        sample = CESP_sample.Sample(index, image, self.interface.channels,
